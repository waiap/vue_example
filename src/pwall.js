...
class Pwall {
  constructor(endpoint, key, secret, resource, mode = "sha256") {
    ...
    request.post(
      ...
      function(error, httpResponse, res) {
        if (
          ...
          var method =
            request_params.body["payload"]["method"] ||
            request_params.body["payload"]["params"]["method"];
          switch (method) {
            case "fpay":
          ...
          request_params.body["payload"]["action"] == "sale" &&
          res["result"]["code"] != "0"
        ) {
          onPaymentError(res);
        }
  ...
  parsePayload(payload, amount, currency, notify_url, group_id) {
    ...
    payload["params"]["token"] = Math.random()
      .toString(36)
      .substring(2, 15);
    payload["params"]["order"] = Math.random()
      .toString(36)
      .substring(2, 15);
  ...
  getHeaders(body) {
    return {
      ...
      "Content-Signature": crypto
        .createHmac(this.mode, this.secret)
